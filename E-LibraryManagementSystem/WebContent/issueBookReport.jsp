...
<body class="main11">
  <div class="div0">
        <div class="div11" >
            <h4>Issue Book Report</h4><hr><br><br>
            <%! int rn=0; %>
              <%
                            Connection con=(Connection)application.getAttribute("oracle");  
                            Statement stmt=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                              ResultSet rs=stmt.executeQuery("SELECT ROWNUM AS SNO,USERID,BOOKID AS ISBN,ISSUES_DATE,RETURN_DATE FROM ISSUES_BOOKS");
                      ...
                      while(rs.next())
                        ...
                        <td><%=rs.getDate("ISSUES_DATE") %></td>
                        <td><%=rs.getDate("RETURN_DATE") %></td>
                        <td><a href="returnBook.jsp" style="text-decoration:none">edit</a></td>
                          </tr>
                          <%
                           rn=rs.getRow();
                            if(rn%15==0)
              ...
              <%ResultSet rs1=stmt.executeQuery("SELECT COUNT(*) FROM ISSUES_BOOKS"); %>
              <p>Total number of books is <%while(rs1.next()) {%> <%=rs1.getInt(1) %><%} %></p>
              <div style="float:right">
               <form action="issueBookReportpn.jsp">
